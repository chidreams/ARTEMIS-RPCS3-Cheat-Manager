...
class PatchFile:
    ...
    def build_cheat_block(self, name, game, serial, version_tag,
        ...
        if style == "Multiple GameIDs and Patches":
            # serial expected as "BLUS12345: 01.00, BLES12345: 01.01" or just comma separated
    ...
    def add_cheat(self, ppu_hash, block_text, anchor_text=None):
        ...
        # Dynamically create or append to the global Anchors node before inserting the PPU block
        if anchor_text:
            anchor_idx = -1
        ...
        target = None
        # We need a clean internal state to reliably find the insertion point of the PPU hash
        temp_pf = PatchFile("\n".join(lines))
...
def launch_gui():
    ...
    class App(tk.Tk):
        def __init__(self):
            ...
            # --- Load known files for [NEW] tracking ---
        ...
        def _build_add_form(self, parent):
            ...
            def add_helper_line():
                ...
                if a and v and a != "Address" and v != "Value":
                    ...
                    # Ensure 0x prefix for Address
                    if not a.lower().startswith("0x"):
                        a = "0x" + a
                    
                    # Ensure 0x prefix for Value (unless it's a float, string, or already negative)
        ...
        def _build_diff_tab(self):
            ...
            # --- Right: side-by-side hex view + AoB output ---
        ...
        def add_cheat(self):
            ...
            if style == "Using anchor (for large patches)":
                # Create an anchor name dynamically based off the cheat name
        ...
        def _refresh_library(self):
            ...
            for fn in sorted(os.listdir(self.library_dir)):
                ...
                # Check if the file is new by comparing against the known files list loaded at startup
